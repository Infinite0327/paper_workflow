...
- `server/`：待上传至服务器的环境检查、CPU 诊断和已授权 GPU 作业模板。
...
3. 用户已持续授权计费 GPU 操作。依赖、数据和脚本核验完成后，可直接提交 `server/gpu_smoke.sbatch`，也可按实验需要使用 `srun` 或交互式 GPU；无须逐次请求批准。
4. 每次 GPU 提交必须保留队列、卡型、卡数、硬时限、预计最高费用、命令、实验目的和作业 ID。默认从 1 张卡和短时冒烟开始，成功后再扩大；禁止无时限运行、空闲占卡和无界自动重试。
